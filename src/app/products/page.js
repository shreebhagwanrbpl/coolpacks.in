        ...
        <div className="max-w-7xl mx-auto px-5">
          ...
          <div className="grid lg:grid-cols-[320px_1fr] gap-10 mt-16">
            {/* ======================
                LEFT SIDEBAR
            ====================== */}
            <aside className="sticky top-28 h-fit rounded-[30px] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-900/5">
              {/* Heading */}
              <div className="mb-6">
                <span className="inline-flex rounded-full bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-800">
                  Browse
                </span>
              ...
              <input
                ...
                className="h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 text-slate-700 outline-none transition-all placeholder:text-slate-400 focus:border-slate-800 focus:bg-white focus:ring-4 focus:ring-slate-800/10"
              />
                    ...
                    <button
                      ...
                      className={`flex w-full items-center justify-between px-5 py-4 font-medium transition-all duration-300 ${
                        activeCategory === category
                          ? "bg-slate-800 text-white shadow-lg shadow-slate-900/15"
                    ...
                    >
                      {groupedProducts[category].map((item) => (
                        <button
                          ...
                          className="block w-full border-t border-slate-100 px-6 py-3 text-left text-sm text-slate-600 transition hover:bg-slate-50 hover:text-slate-900"
                        >
                          {item.title}
                        </button>
                      ))}
                ...
                {Object.entries(groupedProducts).map(([category, list]) => (
                  ...
                  >
                    ...
                    <div className="mb-10 flex items-center justify-between border-b border-slate-200 pb-5">
                      ...
                      <div className="rounded-full border border-slate-200 bg-slate-50 px-5 py-2 text-slate-700 font-semibold">
                    ...
                    <div className="space-y-8">
                      {list.map((product) => (
                        <div
                          key={product.slug}
                          id={product.slug}
                          className="group rounded-[30px] border border-slate-200 bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-2 hover:border-slate-300 hover:shadow-xl hover:shadow-slate-900/5"
                        >
                          <div className="grid items-center gap-8 lg:grid-cols-[250px_1fr_190px]">
                            {/* Image */}
                            <div className="flex h-[220px] items-center justify-center overflow-hidden rounded-3xl border border-slate-100 bg-slate-50">
                              <Image
                                src={product.image}
                                alt={product.title}
                                width={220}
                            ...
                            <div className="flex justify-center lg:justify-end">
                              ...
                              >
                                <button className="w-full rounded-xl bg-slate-800 px-8 py-4 font-semibold text-white transition-all duration-300 hover:bg-slate-900 hover:shadow-lg hover:shadow-slate-900/20 lg:w-auto">
                                  View Details →
                                </button>
                              </Link>
                            </div>
        ...
        <div className="max-w-7xl mx-auto px-5">
          ...
          <div className="mt-16 grid gap-8 md:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div
                key={index}
                className="group rounded-[30px] border border-slate-200 bg-white p-8 text-center shadow-sm transition-all duration-300 hover:-translate-y-2 hover:border-slate-300 hover:shadow-xl hover:shadow-slate-900/5"
              >
                {/* Icon */}
                <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-3xl bg-slate-100 text-slate-800 transition-all duration-300 group-hover:bg-slate-800 group-hover:text-white">
                  {item.icon}
                </div>
                ...
                {/* Bottom Line */}
                <div className="mx-auto mt-8 h-1 w-14 rounded-full bg-slate-800 transition-all duration-300 group-hover:w-24"></div>
              </div>
            ))}
          </div>
        </div>
